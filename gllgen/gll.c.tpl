...
static PFN_vkVoidFunction getproc(const char *procname){
	PFNGLVOIDFUNCTION proc;
	if(l_libgl == NULL){
		LOG_ERROR("gll getproc: OpenGL library is not loaded!");
		return NULL;
	}

	proc = (PFNGLVOIDFUNCTION)GetProcAddress(l_libgl, procname);
	if(proc == NULL)
...
GLboolean GL_LoadAPI(void)
{
	...
	#define GL_GET_PROC_ADDR(proc_type, proc_name)									\
		proc_name = (proc_type) getproc(#proc_name);								\
		if(proc_name == NULL){														\
